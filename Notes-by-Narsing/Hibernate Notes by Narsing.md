...
**Answer:** Hibernate is a object relational mapping framework which
maps the java classes to the database tables and java data types to SQL
data types automatically and generates the queries automatically.
...
**Answer:** ORM stands for object relational mapping and it is a technic
for converting object-oriented programming data to relational database.
...
## What are the advantages of Hibernate over JDBC?
  ...
  ------------------- ------------------- ---------------------------------
  Query Writing       Requires SQL        Uses HQL (Hibernate Query
                      queries             Language)

  Object Mapping      Manual mapping      Automatic ORM mapping

  Caching             No built-in caching Supports first-level and
...
\<!\-- [Hibernate]{.underline} DDL Auto (Update DB Schema) \--\>
...
\<property
name=\"hibernate.connection.url\"\>[jdbc]{.underline}:[sqlserver]{.underline}://DESKTOP-G774017:1433;databaseName=[testdb]{.underline};encrypt=false\</property\>

\<property
name=\"hibernate.connection.username\"\>[Narsing]{.underline}\</property\>
...
@[Table]{.underline}(name = \"Emp\")

**public** **class** [Employee]{.underline} {

@[Id]{.underline}

@[GeneratedValue]{.underline}(strategy =
[GenerationType]{.underline}.IDENTITY)
...
// 1. Load [Hibernate]{.underline} Configuration

[Configuration]{.underline} cfg = **new** [Configuration]{.underline}();

cfg.configure(\"hibernate.cfg.xml\");

cfg.addAnnotatedClass([Employee]{.underline}.**class**);

System.***out***.println(\"Loaded Hibernate configuration\");

// 2. Build SessionFactory

[SessionFactory]{.underline} sessionFactory = cfg.buildSessionFactory();

// 3. Open Session

[Session]{.underline} session = sessionFactory.openSession();

// 4. Start Transaction

[Transaction]{.underline} txn = session.beginTransaction();

// 5. Create Employee Object

[Employee]{.underline} emp = **new** [Employee]{.underline}();
...
- The [Configuration]{.underline} class is used to **load and
  configure** Hibernate settings from hibernate.cfg.xml.

- This takes hibernate-configuration file name and location as input
  value . also take hibernate maping files names.
...
- **Explanation**:
  ...
  - **new** [Configuration]{.underline}() initializes the Hibernate
    configuration.
...
- If you\'re using **annotations** instead of hbm.xml mapping files,
...
- **Explanation**:
  ...
  - This tells Hibernate to recognize the \@Entity annotated
    [Employee]{.underline} class.

### Step 3: Build the SessionFactory Object

- The [SessionFactory]{.underline} is the main Hibernate factory for
  creating sessions:

- It is the interface used to create session factory object and which
  turn configure hibernate for the application using configuration file
  and allows for a session object to be intiate.

> [SessionFactory]{.underline} sessionFactory =
> cfg.buildSessionFactory();

- **Explanation**:

  - buildSessionFactory reads the configuration and creates a **single
    instance** of [SessionFactory]{.underline}.

  - This object **must be created once** (Singleton Pattern).

### Step 4: Open a Session

- A [Session]{.underline} represents a **database connection**.
...
- **Explanation**:
  ...
  - Sessions are **not thread-safe**; use a new one for each
    transaction.
...
- **Explanation**:
  ...
  - All DB changes must be committed or rolled back within a
    transaction.
...
- **Explanation**:
  ...
  - save(emp) inserts a new row if the primary key does not exist;
...
### ✅ Summary of Steps
  ...
  **Step**   **Operation**       **Method Used**
  ---------- ------------------- ---------------------------------------
  1️         Load Hibernate      cfg.configure(\"hibernate.cfg.xml\")
             Config              

  2️         Add Entity Class    cfg.addAnnotatedClass(Employee.class)
             (Optional)          

  3️         Build               cfg.buildSessionFactory()
             SessionFactory      
  ...
  8️         Close Resources     session.close();
                                 sessionFactory.close();
  ----------------------------------------------------------------------
...
**Answer:** SessionFactory is a heavyweight object that creates and
manages Session objects. It is created **once per database** and is
thread-safe.

## What is Session in Hibernate?

**Answer:** A Session is a lightweight, non-thread-safe object that acts
as a bridge between Java code and the database. It is used to perform
CRUD operations.

## What are the different states of an entity in Hibernate?

- **Transient:** The object is created but not associated with a
  Hibernate session.

- **Persistent:** The object is associated with a session and mapped to
  a database.
...
**Answer:** HQL is an object-oriented query language that uses entity
names instead of table names. It is database-independent.\
Example:

String hql = \"FROM Student WHERE name = :name\";

[Query]{.underline} query = session.[~~createQuery~~(hql)]{.underline};

query.setParameter(\"name\", \"Nirav\");

List\<[Student]{.underline}\> students = query.list();

## What are Fetch Types in Hibernate?

- **Lazy Loading (FetchType.LAZY)** -- Data is loaded only when
  requested.
...
@[OneToMany]{.underline}(fetch = [FetchType]{.underline}.LAZY)

**private** List\<[Course]{.underline}\> [courses]{.underline};
...
##  What is the difference between save(), persist(), and saveOrUpdate()?
  ...
  ---------------- ----------------------------------- --------------------
  save()           Inserts new record                  Generated primary
                                                       key

  persist()        Inserts new record, but doesn't     void
                   return ID                           
...
@[OneToMany]{.underline}(mappedBy = \"department\", cascade =
[CascadeType]{.underline}.ALL)

**private** List\<[Employee]{.underline}\> [employees]{.underline};
...
CriteriaQuery\<[Student]{.underline}\> cq =
cb.[createQuery]{.underline}([Student]{.underline}.**class**);

Root\<[Student]{.underline}\> root =
cq.from([Student]{.underline}.**class**);

cq.select(root).where(cb.equal(root.get(\"name\"), \"Nirav\"));

[Query]{.underline}\<[Student]{.underline}\> query =
session.~~createQuery~~(cq);

List\<[Student]{.underline}\> students = query.getResultList();
...
Question q=session.[~~load~~(Question.**class**, 2)]{.underline};
...
**Answer:** Memory leaks can occur because the session holds database
connections and cached objects.
...
In **One-to-One mapping**, one entity is related to exactly one other
entity.\
Example: A **Person** has only **one Passport**, and a Passport belongs
to only one Person.
...
In **One-to-Many mapping**, one entity can be related to multiple other
entities.\
Example: A **Department** has multiple **Employees**, but an Employee
...
In **Many-to-Many mapping**, multiple entities can be associated with
...
**public** **class** [Student]{.underline} {
...
**private** List\<[Course]{.underline}\> courses;
...
**private** List\<[Student]{.underline}\> students;
...
👆 **Student 1 is enrolled in Course 101 and 102, while Student 2 is
only in Course 101.**
...
### Summary Table of Hibernate Mappings
  ...
  One-to-Many    Department ↔        \@OneToMany,
                 Employees           \@ManyToOne
...
✔ **Many-to-Many**: Entities have multiple relationships with each other
via a join table.
...
### Hibernate Mapping Annotations Summary Table
  ...
  **Annotation**                  **Description**   **Example Usage**
  ------------------------------- ----------------- -------------------------------
  \@Entity                        Marks a class as  \@Entity public class Student
                                  a JPA entity      {}
                                  (mapped to a      
                                  table)            

  \@Table(name = \"table_name\")  Specifies the     \@Table(name = \"students\")
                                  table name in the 
                                  database          

  \@Id                            Marks a field as  \@Id private Long id;
                                  the primary key   

  \@GeneratedValue(strategy =     Specifies how the \@GeneratedValue(strategy =
  GenerationType.IDENTITY)        primary key is    GenerationType.IDENTITY)
                                  generated         

  \@Column(name =                 Maps a field to a \@Column(name =
  \"column_name\")                specific database \"student_name\") private
                                  column            String name;
...
- It is a part of spring framework that reduces the boiler plate code of
  database connection and transection management and take care of
  everything. We have to implement only repository interface and it will
  handle every thing

- It has inbuild methods to perform database operations like findbyId,
  findall() etc.

- It is built on top of JPA and provide the repository interface that
...
**Answer :** The \@Entity annotation marks a Java class as a **JPA
entity**, meaning it maps to a table in the database.

### What is the \@Repository annotation?

It is used in Spring Data JPA to indicate that a class is a **DAO (Data
Access Object)**. It also enables **exception translation** for
...
## What is the difference between JpaRepository, CrudRepository, and PagingAndSortingRepository?
  ...
  PagingAndSortingRepository   Extends CrudRepository with pagination and
                               sorting features.

  JpaRepository                Extends PagingAndSortingRepository and provides
                               **flush, batch processing, and custom queries**.
  ------------------------------------------------------------------------------
...
**public** String listTasks(@RequestParam(name = \"page\", defaultValue
= \"0\") **int** page,
...
Page\<Tasks\> tasksPage = taskserviceImpl.alltasks(PageRequest.of(page,
size));
...
void updateSalary(@Param(\"salary\") Double salary, \@Param(\"id\") Long
id);
...
Pagination allows you to **fetch data in smaller chunks (pages)**
instead of retrieving all records at once, improving performance.
...
**public** Page\<Product\> getMethodName(@RequestParam(defaultValue =
\"0\") **int** pageNo,
...
##  Pagination Methods in Page Interface
  ...
  Method               Description
  -------------------- ---------------------------------
  getContent()         Retrieves the current page
                       content as a List\<T\>.

  getTotalPages()      Returns the total number of pages
                       available.

  getTotalElements()   Returns the total number of
                       records.

  getSize()            Returns the number of elements
                       per page.

  getNumber()          Returns the **current** page
                       index.

  hasNext()            Returns true if the next page
                       exists.

  hasPrevious()        Returns true if a previous page
                       exists.
  ------------------------------------------------------
...
Sorting allows us to **order database records based on specific
fields**.
...
getPaginatedAndSortedProducts(@RequestParam(defaultValue = \"0\")
**int** pageNumber,
...
**public** Page\<Product\> pageAndSortProducts(**int** pageNumber,
...
List\<Employee\> findEmployeesSorted(@Param(\"department\") String
department);
...
## 🔹 Sorting Methods in Sort Interface
  ...
  Method                             Description
  ---------------------------------- -------------------------
  Sort.by(\"name\").ascending()      Sorts by name in
                                     **ascending** order.

  Sort.by(\"salary\").descending()   Sorts by salary in
                                     **descending** order.

  Sort.by(\"name\", \"salary\")      Sorts by name first, then
                                     salary.
  ------------------------------------------------------------
...
# 🔹 Summary
  ...
  --------------- ---------------------- ---------------------------------
  Purpose         Fetch limited records  Order records in
                  per page               ascending/descending order
  ...
  Methods Used    PageRequest.of(page,   Sort.by(\"column\").ascending()
                  size)                  
