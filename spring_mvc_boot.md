...
```text
Multiple requests → same final result
```
...
Deleting multiple times → same result.
...
If request repeats → return previous response.
...
Duplicate request → database rejects.

------

#### 3. Cache (Redis)

Use cache to store request IDs.
...
```text
MessageID → saved in DB
If same message comes → skip processing
```

------

Idempotency means executing the same request multiple times results in the same outcome as executing it once. It is used to prevent duplicate operations in distributed systems. We usually implement it using idempotency keys, database unique constraints, or caching mechanisms like Redis.
